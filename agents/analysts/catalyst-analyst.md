...
| 脚本 | 功能 | 用法 |
|------|------|------|
| `fetch-theme-pool.sh` | 获取题材池排行 | `./fetch-theme-pool.sh <trade_date> [limit]` |
| `fetch-theme-stocks.sh` | 获取题材成分股 | `./fetch-theme-stocks.sh <theme_name> <trade_date>` |
...
- 选取 2-3 篇高价值内容深度阅读
- 提炼发现、识别空白、调整下一轮搜索方向
- **必须逐轮填写，不得跳轮，不得在 Round 10 完成前输出结论**
...
- Round 9-10: 搜索风险因素 + 持续性判断

### 步骤 3: 汇总所有题材研究，输出最终报告

## 10 轮研究模板（每个题材）

必须严格按照以下格式填写，逐轮完成，不得跳过：
...
3. 使用 chrome-cdp 脚本进行 10 轮迭代研究
4. 最后按照输出格式生成完整报告

**重要提醒**：
- 必须完成全部 10 轮研究才能输出最终结论
- 每轮研究必须填写搜索词、阅读内容、关键发现、信息空白、下一轮方向
- 不得跳轮，不得在 Round 10 之前输出结论性内容
